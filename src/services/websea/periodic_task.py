...
async def periodic_task(execution_timestamp: str):
    ...
    with timeit_context("websea full execution"):
        with timeit_context("websea scraping"):
            latest_tickers_data_points = await scrape_update_db(execution_timestamp)
            print(f"websea n_symbols: {len(latest_tickers_data_points)}")
        with timeit_context("websea notifs"):
    ...
    latest_tickers_my_format: List[TickerAnalyticsDataPoint] = [
        {
            ...
            "last_price": float(dp["symbol_detail"]["newPrice"]),
            "index_price": float(dp["symbol_detail"]["indexPrice"]),
            "mark_price": float(dp["symbol_detail"]["markerPrice"]),
            "funding_rate": float(dp["symbol_detail"]["capitalRate"]),
            "leverage_max": int(dp["symbol_detail"]["maxMultiple"]),
            "index_mark_delta_div_index": (
                float(dp["symbol_detail"]["indexPrice"])
                - float(dp["symbol_detail"]["markerPrice"])
            )
            / float(dp["symbol_detail"]["indexPrice"]),
            "mark_last_delta_div_mark": (
                float(dp["symbol_detail"]["markerPrice"])
                - float(dp["symbol_detail"]["newPrice"])
            )
            / float(dp["symbol_detail"]["markerPrice"]),
        }
