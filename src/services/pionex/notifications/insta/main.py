...
from src.services.pionex.notifications.insta.notifications.funding_rate_neg_1 import (
    get_notif_to_fire as get_notif_to_fire_funding_rate_neg,
)
from src.services.pionex.notifications.insta.notifications.index_mark import (
    get_notif_to_fire as get_notif_to_fire_index_mark,
)
from src.services.pionex.notifications.insta.notifications.mark_last import (
...
def last_30_ticks_table_url_template(symbol: str):
    return f"{config['back_url']}/pionex/n_last_ticks_table?symbol={symbol}&n=30"
...
"""
    ...
    if should_send_notif_rate_limit(full_notif_name):
        send_message_broadcast_chats(
            message_to_send, ["all", "prior", "individ_pionex"]
        )
