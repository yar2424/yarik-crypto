...
from src.db.repositories.bitvenus.TickerTimeseries import get_ticker_timeseries
from src.services.bitvenus.scrape.get_ticker_data import (
    get_all_symbols,
    get_tickers_data,
)
...
@router.get("/scrape", tags=["bitvenus"])
async def scrape_all():
    return await get_tickers_data()


@router.get("/pairs", tags=["bitvenus"])
async def register_session():
    all_symbols = get_all_symbols()
    return all_symbols


@router.get("/n_last_ticks", tags=["bitvenus"])
async def n_last_ticks(symbol: str, n: int):
    return get_ticker_timeseries(symbol, steps=n)


@router.get("/n_last_ticks_table", tags=["bitvenus"])
...
@router.get("/n_last_ticks_csv", tags=["bitvenus"])
