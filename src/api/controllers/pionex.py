...
from src.db.repositories.pionex.TickerTimeseries import get_ticker_timeseries
from src.services.pionex.scrape.get_ticker_data import get_all_symbols, get_tickers_data
from src.utils.telegram import send_message_broadcast

router = APIRouter()


# @router.get("/scrape/{symbol}", tags=["pionex"])
# async def scrape_symbol(symbol: str):
#     return await get_ticker_data(symbol)


@router.get("/scrape", tags=["pionex"])
async def scrape_all():
    return await get_tickers_data()


@router.get("/pairs", tags=["pionex"])
async def register_session():
    all_symbols = get_all_symbols()
    return all_symbols


@router.get("/n_last_ticks", tags=["pionex"])
async def n_last_ticks(symbol: str, n: int):
    return reversed(get_ticker_timeseries(symbol, steps=n))


@router.get("/n_last_ticks_table", tags=["pionex"])
...
@router.get("/n_last_ticks_csv", tags=["pionex"])
