...
from src.db.repositories.toobit.TickerTimeseries import get_ticker_timeseries
from src.services.toobit.scrape.get_ticker_data import get_all_symbols, get_tickers_data
from src.utils.telegram import send_message_broadcast

router = APIRouter()


@router.get("/scrape", tags=["toobit"])
async def scrape_all():
    return await get_tickers_data()


@router.get("/pairs", tags=["toobit"])
async def register_session():
    all_symbols = get_all_symbols()
    return all_symbols


@router.get("/n_last_ticks", tags=["toobit"])
async def n_last_ticks(symbol: str, n: int):
    return get_ticker_timeseries(symbol, steps=n)


@router.get("/n_last_ticks_table", tags=["toobit"])
...
@router.get("/n_last_ticks_csv", tags=["toobit"])
